    ...
    public void takeAmount(double value) {
        ...
        if (value > 0) {
            ...
            if (elapsed >=1) {
                ...
                this.addHistoryRecord(String.format("Неудачная попытка снятия средств на сумму: %.2f. Дата последнего пополнения: %s.", value, this.lastPutDateTime));
            }
        }
        else {
            this.addHistoryRecord(String.format("Неудачная попытка снятия средств на сумму: %.2f. Текущий баланс: %.2f.", value, this.getAmountProtected()));
        }
    }
